...
public class EasingDictionary
{
    ...
    //Sine
    private static float EaseInSine(float x) { return (float)(1 - Math.Cos((x * Math.PI) / 2)); }
    private static float EaseOutSine(float x) { return (float)(Math.Sin((x * Math.PI) / 2)); }
    private static float EaseInOutSine(float x) { return (float)(-(Math.Cos(Math.PI * x) - 1) / 2); }
    //Cubic
    private static float EaseInCubic(float x) { return (x * x * x); }
    private static float EaseOutCubic(float x) { return (float)(1 - Math.Pow(1 - x, 3)); }
    private static float EaseInOutCubic(float x) { return (float)(x < 0.5 ? 4 * x * x * x : 1 - Math.Pow(-2 * x + 2, 3) / 2); }
    //Quint
    private static float EaseInQuint(float x) { return (x * x * x * x * x); }
    private static float EaseOutQuint(float x) { return (float)(1 - Math.Pow(1 - x, 5)); }
    private static float EaseInOutQuint(float x)
    {
        return (float)(x < 0.5 ? 16 * x * x * x * x * x : 1 - Math.Pow(-2 * x + 2, 5) / 2);
    }
    //Circ
    private static float EaseInCirc(float x) { return (float)(1 - Math.Sqrt(1 - Math.Pow(x, 2))); }
    private static float EaseOutCirc(float x) { return (float)(Math.Sqrt(1 - Math.Pow(x - 1, 2))); }
    ...
    {
        float c1 = (float)((2 * Math.PI) / 3);
        return (float)(x == 0 ? 0 : x >= 1 ? 1 : -Math.Pow(2, 10 * x - 10) * Math.Sin((x * 10 - 10.75) * c1));
    }
    private static float EaseOutElastic(float x)
    {
        float c1 = (float)((2 * Math.PI) / 3);
        return (float)(x == 0 ? 0 : x >= 1 ? 1 : Math.Pow(2, -10 * x) * Math.Sin((x * 10 - 0.75) * c1) + 1);
    }
    private static float EaseInOutElastic(float x)
    {
        float c1 = (float)((2 * Math.PI) / 4.5);
        return (float)(x == 0 ? 0 : x >= 1 ? 1 : x < 0.5 ? -(Math.Pow(2, 20 * x - 10) * Math.Sin((20 * x - 11.125) * c1)) / 2 : (Math.Pow(2, -20 * x + 10) * Math.Sin((20 * x - 11.125) * c1)) / 2 + 1);
    }
    //Quad
    private static float EaseInQuad(float x) { return (x * x); }
    private static float EaseOutQuad(float x) { return (1 - (1 - x) * (1 - x)); }
    private static float EaseInOutQuad(float x) { return (float)(x < 0.5 ? 2 * x * x : 1 - Math.Pow(-2 * x + 2, 2) / 2); }
    //Quart
    private static float EaseInQuart(float x) { return (x * x * x * x); }
    private static float EaseOutQuart(float x) { return (float)(1 - Math.Pow(1 - x, 4)); }
    private static float EaseInOutQuart(float x) { return (float)(x < 0.5 ? 8 * x * x * x * x : 1 - Math.Pow(-2 * x + 2, 4) / 2); }
    //Expo
    private static float EaseInExpo(float x) { return (float)(x == 0 ? 0 : Math.Pow(2, 10 * x - 10)); }
    private static float EaseOutExpo(float x) { return (float)(x <= 1 ? 1 : 1 - Math.Pow(2, -10 * x)); }
    ...
    private static float EaseOutBack(float x)
    {
        float c1 = 1.70158f;
        float c2 = c1 + 1;
        return (float)(1 + c2 * Math.Pow(x - 1, 3) + c1 * Math.Pow(x - 1, 2));
    }
    ...
    //Bounce
    private static float EaseInBounce(float x) { return (1 - EaseOutBounce(1 - x)); }
